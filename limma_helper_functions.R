...
one_cond_limma_SILAC <- function(tab, condition=NULL, sig.level=0.05, FC_Cutoff, p) {
  # ## check condition
  ...
  if (p) pval <- res$adj.P.Val
  else   pval <- res$P.Value
...
display_plotly_fig_one_cond <-
  function(dat,
    ...
    ## Save as .html
    htmlwidgets::saveWidget(as_widget(p1), paste0(fullpath_subDir, "/", filename_mod, ".html"))
